...
class Postgresql {
    ...
    function db_update($schema_name, $table_name, $record = NULL, $entry_on_field = NULL) {
        $values = array();
        $prepare = array();
        $count = 0;
        ...
        if (!is_null($entry_on_field)) {
            $prepare[] = $entry_on_field . ' = coalesce(' . $entry_on_field . ', current_timestamp)';
        }
        $values[] = $record['id'];
    ...
    function is_user_in_role($user_name, $role_name) {
        $result = $this->db_query_params(
                'SELECT dbo.is_user_in_role($1, $2);',
                array($user_name, $role_name));
        if ($row = pg_fetch_row($result)) {
            return $row[0];
        }
        throw new Exception('Unable to determine if user is authorized.');
